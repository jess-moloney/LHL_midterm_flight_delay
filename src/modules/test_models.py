...
class test_models:
    ...
    def split_scale(self, data, target = None):
        ...
        """
        if target is None:
            X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(data, random_state=123)
        else:
            X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(data, target, random_state=123)
